...
def name_gen_prob(**kwargs):
    ...
    dist[
        16
    ] += diff  # add any residual required probability to q, for that fantasy name panache # noqa: E501
...
def name_gen_alternator(**kwargs):
    ...
    cons_seq_len = 0
    while len(name) < name_len:
        choice_num = numpy.random.choice([0, 1], p=[v_prob, cons_prob])
        ...
        if last_letter in VOWELS:
            ...
            v_prob -= round(0.05 * v_seq_len, 2)
            cons_prob += round(0.05 * v_seq_len, 2)
            ...
            v_prob += round(0.05 * cons_seq_len, 2)
            cons_prob -= round(0.05 * cons_seq_len, 2)
